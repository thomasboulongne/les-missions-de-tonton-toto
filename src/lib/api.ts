...
export async function getMissionSubmission(
  ...
  const submissions = await getSubmissions(missionId);
  // Return the most recent submission (already sorted by submitted_at DESC from backend)
...
export async function getUnreadReviews(
  since: string
): Promise<SubmissionWithMission[]> {
  const res = await fetch(
    `${API_BASE}/submissions?reviewed_since=${encodeURIComponent(since)}`
  );
...
export function getRevealedHints(missionId: number): {
  hint1: boolean;
  hint2: boolean;
} {
